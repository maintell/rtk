...
mod tests {
    ...
    fn test_ps_get_content_unmappable_params_pass_through() {
        // These have no faithful `rtk read` translation, so the cmdlet translator
        // declines them (tested in isolation here). End-to-end they then fall
        // back to `rtk powershell Get-Content …` via PR #2881's RULES — see
        // `test_ps_unmappable_cmdlet_falls_back_to_rtk_powershell`.
        assert_eq!(rewrite_powershell("Get-Content foo.txt -Raw"), None);
        assert_eq!(
            rewrite_powershell("Get-Content foo.txt -Encoding utf8"),
            None
        );
        // Head and tail at once has no single rtk read invocation.
        assert_eq!(
            rewrite_powershell("Get-Content foo.txt -TotalCount 5 -Tail 5"),
            None
        );
        // Multiple files: --max-lines takes one positional path.
        assert_eq!(rewrite_powershell("Get-Content a.txt b.txt"), None);
    }
    ...
    fn test_ps_get_childitem_unmappable_params_pass_through() {
        assert_eq!(
            rewrite_powershell("Get-ChildItem -ErrorAction SilentlyContinue"),
            None
        );
        // `-Include a,b` needs `find ( -name a -o -name b )`.
        assert_eq!(
            rewrite_powershell("Get-ChildItem -Recurse -Include *.rs,*.toml"),
            None
        );
        // -File and -Directory are mutually exclusive.
        assert_eq!(
            rewrite_powershell("Get-ChildItem -Recurse -File -Directory"),
            None
        );
    }
    ...
    #[test]
    fn test_ps_unmappable_cmdlet_falls_back_to_rtk_powershell() {
        // End-to-end, a cmdlet the translator (PR #3439) declines still matches
        // `rtk powershell <Cmdlet>` in PR #2881's RULES table, so it routes
        // through rtk's faithful passthrough rather than running raw PowerShell.
        assert_eq!(
            ps("Get-Content foo.txt -Raw"),
            Some("rtk powershell Get-Content foo.txt -Raw".into())
        );
        assert_eq!(
            ps("Select-String -Pattern bar"),
            Some("rtk powershell Select-String -Pattern bar".into())
        );
    }
